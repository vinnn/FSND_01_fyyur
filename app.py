...
# call init_app to initialise (reminder: SQLAlchemy db was not initialised in models.py)
db.init_app(app)
...
def venues():
  ...
  for place in places:
    ...
    venues_in_place = Venue.query.filter(Venue.city==place.city).filter(Venue.state==place.state).all()
    ...
    for venue_in_place in venues_in_place:
      ...
      dict_venue['num_upcoming_shows']=Show.query.filter(Show.venue_id==venue_in_place.id).filter(Show.start_time>datetime.now()).count()
...
def search_venues():
  ...
  for found_venue in found_venues:
    ...
    num_upcoming = Show.query.filter(Show.start_time>datetime.now()).filter(Show.venue_id==found_venue.id).count()
...
def show_venue(venue_id):
  ...
  past_count = Show.query.filter(Show.venue_id==venue_id).filter(Show.start_time<datetime.now()).distinct(Show.artist_id).count()
  upcoming_count = Show.query.filter(Show.venue_id==venue_id).filter(Show.start_time>datetime.now()).distinct(Show.artist_id).count()
  dict_venue["past_shows_count"] = past_count
  dict_venue["upcoming_shows_count"] = upcoming_count

  #dict_venue["past_shows_count"] = len(dict_venue["past_shows"])
  #dict_venue["upcoming_shows_count"] = len(dict_venue["upcoming_shows"])
...
def create_venue_submission():
  ...
  # on successful db insert, flash success
    ...
    flash('An error occurred. The creation input for Venue ' + request.form['name'] + ' were not all valid.')
...
def search_artists():
  ...
  for found_artist in found_artists:
    ...
    num_upcoming = Show.query.filter(Show.start_time>datetime.now()).filter(Show.artist_id==found_artist.id).count()
...
def show_artist(artist_id):
  ...
  past_count = Show.query.filter(Show.artist_id==artist_id).filter(Show.start_time<datetime.now()).distinct(Show.venue_id).count()
  upcoming_count = Show.query.filter(Show.artist_id==artist_id).filter(Show.start_time>datetime.now()).distinct(Show.venue_id).count()
...
def edit_venue_submission(venue_id):
  ...
  venue = Venue.query.filter_by(id=venue_id).first_or_404()
  form = VanueForm(request.form, meta={'csrf': False})     # 
  if form.validate_on_submit():
    error = False
    try:
...
def create_artist_submission():
  ...
  # on successful db insert, flash success
    ...
    flash('An error occurred. The creation input for Artist ' + request.form['name'] + ' were not all valid.')
...
def shows():
  ...
  for show in shows:
    ...
    data_dict["venue_name"] = Venue.query.filter(Venue.id==show.venue_id)[0].name
    data_dict["artist_id"] = show.artist_id
    data_dict["artist_name"] = Artist.query.filter(Artist.id==show.artist_id)[0].name
    data_dict["artist_image_link"] = Artist.query.filter(Artist.id==show.artist_id)[0].image_link
    data_dict["start_time"] = show.start_time.strftime("%d/%m/%Y, %H:%M") 
    data_list.append(data_dict)
